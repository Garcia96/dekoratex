  ...
  <div class="modal fade" id="ventana1">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <?php
          $cod= mysqli_query($conexion, "select cod from user where cod ='".$_GET['valide']."'");

          if($codigo = mysqli_fetch_array($cod)){
          $query = mysqli_query($conexion,"select * from user where cod = '".$_GET['valide']."'");
          if($i = mysqli_fetch_array($query)){
            if(isset($_POST['confirmar'])){
                mysqli_query($conexion,"update user set state = 1 where email = '".$i['email']."'");
            ...
            echo '<div class="container registro">
                    <div class="col-md-12 "  style="padding: 5%;">
                        <div class="form-group">
                          <h3>Bienvenido '.$i['Name'].'</h3><br>
                          <form action="" method="post">
                            <label for="">Gracias por registrarse en Dekoratex, para poder continuar es
                            necesario validar su cuenta</label><br>
                            <input name="confirmar" value="Validar" class="btn btn-primary center-block" type="submit">
                          </form>
                      </div>
                    </div>
                  </div>';
